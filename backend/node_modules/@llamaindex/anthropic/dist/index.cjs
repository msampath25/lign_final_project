...
class AnthropicSession {
    constructor(options = {}){
        if (!options.apiKey) {
            options.apiKey = env.getEnv("ANTHROPIC_API_KEY");
        }
        if (!options.apiKey) {
            throw new Error("Set Anthropic Key in ANTHROPIC_API_KEY env variable");
        }
        this.anthropic = new sdk.Anthropic(options);
    }
}
 ...
 */ function getAnthropicSession(options = {}) {
    let session = defaultAnthropicSession.find((session)=>{
        return remeda.isDeepEqual(session.options, options);
    })?.session;
    if (!session) {
...
class Anthropic extends llms.ToolCallLLM {
    static{
    ...
    formatMessages(messages) {
        const result = messages.filter((message)=>message.role === "user" || message.role === "assistant").map((message)=>{
            ...
            if ("toolResult" in options) {
                    ...
                    content: [
                        {
                            ...
                            content: [
                                {
                                    type: "text",
                                    text: utils.extractText(message.content)
                                }
                            ],
                            tool_use_id: id
                        }
                ...
                const aiThinkingText = utils.extractText(message.content);
                return {
                    role: "assistant",
                    content: [
            ...
            return {
                content: typeof message.content === "string" ? message.content : message.content.map((content)=>content.type === "text" ? {
                        type: "text",
                        text: content.text
                    } : {
                        type: "image",
    ...
    async chat(params) {
        ...
        if (systemMessages.length > 0) {
            systemPrompt = systemMessages.map((message)=>message.options && "cache_control" in message.options ? {
                    type: "text",
                    text: utils.extractText(message.content),
        ...
        const beta = systemPrompt?.find((message)=>"cache_control" in message) !== undefined;
        // case: Non-streaming
    ...
    async *streamChat(messages, systemPrompt, anthropic) {
        ...
        for await (const part of stream){
            const content = part.type === "content_block_delta" ? part.delta.type === "text_delta" ? part.delta.text : part.delta : undefined;
...
class AnthropicAgentWorker extends agent.LLMAgentWorker {
}
class AnthropicAgent extends agent.LLMAgent {
    constructor(params){
        const llm = params.llm ?? (global.Settings.llm instanceof Anthropic ? global.Settings.llm : new Anthropic());
        super({
            ...params,
            llm
        });
...
exports.ALL_AVAILABLE_ANTHROPIC_LEGACY_MODELS = ALL_AVAILABLE_ANTHROPIC_LEGACY_MODELS;
exports.ALL_AVAILABLE_ANTHROPIC_MODELS = ALL_AVAILABLE_ANTHROPIC_MODELS;
exports.ALL_AVAILABLE_V3_5_MODELS = ALL_AVAILABLE_V3_5_MODELS;
exports.ALL_AVAILABLE_V3_MODELS = ALL_AVAILABLE_V3_MODELS;
exports.Anthropic = Anthropic;
exports.AnthropicAgent = AnthropicAgent;
exports.AnthropicAgentWorker = AnthropicAgentWorker;
